...
if (isset($_POST['unarchive_id'])) {
    ...
    try {
        ...
        $query = "UPDATE `general_deworming` 
                  SET `is_archived` = 0, 
                      `archived_at` = NULL, 
                      `archived_by` = NULL,
                      `archive_reason` = NULL 
        ...
        if ($stmt->execute()) {
            $con->commit();
            $message = 'Deworming record unarchived successfully.';
        } else {
            $con->rollback();
            $message = 'Failed to unarchive deworming record.';
        }
...
header("Location: ../general_deworming.php?message=" . urlencode($message));
exit;
?> 