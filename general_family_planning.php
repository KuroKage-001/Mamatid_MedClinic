...
// Retrieve family planning records based on archive status
try {
    if ($show_archived) {
        $query = "SELECT `id`, `name`, `address`, `age`,
                         ...
                         `archive_reason`,
                         (SELECT `display_name` FROM `admin_user_accounts` WHERE `id` = `general_family_planning`.`archived_by`) as `archived_by_name`
...
<head>
  ...
  <style>
    ...
    #all_family_wrapper .row:first-child {
      margin-bottom: 15px;
    }

    #all_family_wrapper .dataTables_filter {
      float: left !important;
      text-align: left !important;
    }

    #all_family_wrapper .dataTables_filter input {
      width: 300px;
      border-radius: 8px;
      border: 2px solid #e4e6ef;
      padding: 0.625rem 1rem;
    ...
    #all_family_wrapper .dataTables_filter input:focus {
      border-color: var(--primary-color);
      box-shadow: 0 0 0 0.2rem rgba(54, 153, 255, 0.25);
    }
  ...
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      <section class="content">
        <div class="card card-outline card-primary">
          <div class="card-header">
            <h3 class="card-title">
              <?php echo $show_archived ? 'Archived Family Planning Records' : 'Active Family Planning Records'; ?>
            </h3>
            <div class="card-tools">
              <div class="d-flex align-items-center">
                <a href="general_family_planning.php" 
                   class="archive-filter-btn <?php echo !$show_archived ? 'active' : ''; ?>">
                  <i class="fas fa-list"></i> Active Records
                </a>
                <a href="general_family_planning.php?show_archived=1" 
                   class="archive-filter-btn <?php echo $show_archived ? 'active' : ''; ?>">
    ...
    <script>
      ...
      function archiveRecord(id, name) {
        Swal.fire({
          title: 'Archive Family Planning Record',
          html: `
          ...
          if (result.isConfirmed) {
            ...
            fetch('actions/archive_family_planning.php', {
              method: 'POST',
              headers: {
      ...
      function unarchiveRecord(id, name) {
        Swal.fire({
          title: 'Unarchive Family Planning Record',
          ...
          if (result.isConfirmed) {
            ...
            fetch('actions/unarchive_family_planning.php', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
